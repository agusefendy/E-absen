      ...
      <div class="content">
        <div class="row" id="tq">
          ...
          if(isset($_POST['tambah'])){
            ...
            $query = mysqli_query($koneksi, "INSERT into kelas(kelas,idguru) values ('$kelas','$pimpinan')");
            if($query){ ?>
              <script>
                swal({
                  position: 'center',
          ...
          //edit
          if(isset($_POST['edit'])){
            $idkelasedit = $_POST['idkelasedit'];
            $kelas = $_POST['kelas'];
            $pimpinan = $_POST['pimpinan'];
            $query = mysqli_query($koneksi, "UPDATE kelas set kelas='$kelas', idguru='$pimpinan' where idkelas='$idkelasedit'");
            if($query){ ?>
              <script>
                swal({
                  position: 'center',
          ...
          if(isset($_GET['idkelas'])){ 
            ...
            $tm = mysqli_query($koneksi, "SELECT * from siswa where idkelas='$idkelas'");
            while($mt = mysqli_fetch_array($tm)){
              $hapusabsen = mysqli_query($koneksi, "DELETE from absen where idsiswa='$mt[idsiswa]'");
            }
            $hapussiswa = mysqli_query($koneksi, "DELETE from siswa where idkelas='$idkelas'");
            $hapuskelas = mysqli_query($koneksi, "DELETE from kelas where idkelas='$idkelas'");
            if($hapuskelas){ ?>
              <script>
                swal({
                  position: 'center',
                  type: 'success',
                  title: '<?php echo "$asset[tipekelompok]"; ?> Dihapus',
                  showConfirmButton: false,
                  timer: 4500
                })
                </script>
            ...
            <?php }else{ ?>
               <script>
                swal({
                  position: 'center',
                  type: 'error',
                  title: 'Gagal Menghapus <?php echo "$asset[tipekelompok]"; ?> :(',
                  showConfirmButton: false,
                  timer: 4500
                })
                </script>
